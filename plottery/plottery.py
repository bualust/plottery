...
def getdata(yaml_data, mc_weight, selection_cuts, treename):
    ...
    for proc in yaml_data["Processes"]["File"]:
        ...
        append_tree = []
        for ff in filenames:
            append_tree.append(ff + ":" + treename)
    ...
    for myfiles, proc in zip(FilesToOpen, yaml_data["Processes"]["Name"]):
        if proc == "Data":
            variables = [item for item in variables if item != mc_weight]
    ...
    for proc in yaml_data["Processes"]["Name"]:
        ...
        for i in range(len(yaml_var)):
            if yaml_var[i]["Index"] >= 0:
                flat_vector(
                    process_dic, yaml_var[i]["Index"], yaml_var[i]["branch_name"]
                )
    ...
    for proc in yaml_data["Processes"]["Name"]:
        for var_rm in var_to_remove:
            del data[proc][var_rm]
...
def selection_to_variables(selection):
    ...
    new_string = re.sub(
        pattern, ",", sample_sel
    )  # replace matched substrings with comma
    new_string = re.sub(
        r"(and|or),", ",", new_string
    )  # remove any remaining and/or followed by comma
    ...
    new_list = [
        s for s in new_list if s != "" and s not in ["and", "or"]
    ]  # remove empty strings, and 'and'/'or' from the list
    return new_list
...
def select(data, yaml_data, selection_cuts, sel_var_vectors):
    ...
    for proc, sel in zip(
        yaml_data["Processes"]["Name"], yaml_data["Processes"]["Selection"]
    ):
        process_dic = data[proc]
...
def plotting(yaml_data):
    ...
    for var, var_name, var_nbins, var_low, var_high, VarBinning in zip(
        Variables,
        VariablesTitle,
        Varriables_nbins,
        VariablesBinLowEdge,
        ...
        histos = {}
        proc_yields = []
        for proc, rescale in zip(
            yaml_data["Processes"]["Name"], yaml_data["Processes"]["Rescale"]
        ):
            ...
            if mc_weight == "" or proc == "Data":
                ...
                process_weight = process_dic[mc_weight]
            lumi_scale = lumi
            if proc == "Data":
                lumi_scale = 1
            ...
            this_yield = 0
            if mc_weight == "" or proc == "Data":
                for ev in process_var:
                    this_yield = this_yield + lumi_scale
            else:
                for weight in process_weight:
                    this_yield = this_yield + weight * lumi_scale
            if proc == "Data":
                hep.histplot(
                    hist,
                    yerr=True,
            ...
            hep.atlas.label(
                data=True, lumi=lumi, label="Internal", rlabel=plot_label, ax=main
            )
            histos[proc] = hist
        ...
        for i_hist in histos:
            ...
            if len(yaml_data["Processes"]["Name"]) == 1:
                vals_ratio = num_hist.values()
            else:
                if normalise:
                    vals_ratio = (num_hist.values() / ref_hist.values()) * (
                        proc_yields[0] / proc_yields[ii]
                    )
                else:
                    vals_ratio = num_hist.values() / ref_hist.values()
            if proc == "Data":
